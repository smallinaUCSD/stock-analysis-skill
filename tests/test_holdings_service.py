...
def _svc(tmp_path):
    ...
    # deterministic + offline: no live price/dividend fetch in unit tests (values
    # fall back to the CSV market_value, so shares stay None and totals are exact).
    svc._prices = lambda tickers: {}
    svc._dividends = lambda tickers: {}
    return svc, p
